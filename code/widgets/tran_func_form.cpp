...
TranFuncForm::TranFuncForm(std::size_t n, std::size_t m, const QString& title, QWidget* parent)
    ...
    pasteBtn_ = new QPushButton(QStringLiteral("⬇"), this); // insert / paste
...
void TranFuncForm::showEvent(QShowEvent* event) {
    ...
    QTimer::singleShot(0, this, &TranFuncForm::repositionClipButtons);
}
...
QLayout* TranFuncForm::createCoeffRow(VecLine& lineEdits, QDoubleValidator* validator, int factor) {
    ...
    for (auto& lineEdit : lineEdits) {
        ...
        connect(lineEdit, &QLineEdit::textChanged, this, [lineEdit, label] {
            ...
            const double value = getValue(lineEdit->text());
            applyPowerActive(label, value != 0);
            applyCoeffSign(lineEdit, value);
...
void TranFuncForm::setFieldValue(LineEdit* lineEdit, double value) {
    lineEdit->setText(formatCoeff(value));
    applyCoeffSign(lineEdit, value);
    adjustLineEditWidth(lineEdit);
}
...
void TranFuncForm::rebuildActivityMask() {
    ...
    for (int p = 0; p < n; ++p) {
        if (getValue(numerator_[static_cast<std::size_t>(p)]->text()) != 0.0) {
    ...
    for (int p = 0; p < static_cast<int>(denominator_.size()); ++p) {
        ...
        if (getValue(denominator_[static_cast<std::size_t>(p)]->text()) != 0.0) {
            is_active_[index] = true;
            id_ += (1 << n) << p;
        }
    }
...
QString TranFuncForm::exportText() const {
    ...
    for (double v : num)
        numParts << QString::number(v, 'g', 12);
    for (double v : den)
        denParts << QString::number(v, 'g', 12);

    QString human = QStringLiteral("W(p) = (%1) / (%2)").arg(polyToString(num), polyToString(den));
    if (tau != 0.0)
        human += QStringLiteral(" · e^(−%1 p)").arg(QString::number(tau, 'f', 2));
    ...
    return QStringLiteral(
               ...
               "\n"
               "%4\n")
        .arg(numParts.join(QLatin1Char(' ')), denParts.join(QLatin1Char(' ')), QString::number(tau, 'f', 2), human);
}
    ...
    if (!importText(text)) {
        QMessageBox::information(this, tr("Вставка ПФ"),
                                 tr("В буфере нет данных формата RegValve-TF-v1.\n"
                                    "Скопируйте ПФ кнопкой ⎘ (копировать) на вкладке «Анализ» или «Синтез»."));
        return;
    }
    if (pasteBtn_)
